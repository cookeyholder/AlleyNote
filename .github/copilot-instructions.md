...
1. **開發前**：
    ...
    - 撰寫測試（TDD）
        ...
        - 測試案例要能快速執行
        - 測試案例要能夠清楚地表達其意圖，並提供足夠的上下文資訊
        - 撰寫完測試檔案以後，就先執行 PHPStan 和 PHP CS Fixer 檢查測試檔案本身是否有問題
    - 撰寫一個腳本工具，掃瞄專案架構並產生報告，幫助理解專案結構與模組依賴關係，還有測試的錯誤類型和數量，每次開發前都至少要執行一次這個腳本，並且參考報告來進行調整。在開發過程如果可以，經常執行腳本讓自己對專案的了解維持在最新。
