...
async function waitForCKEditorReady(page, containerSelector = "#content") {
  ...
  await page.waitForFunction(() => typeof ClassicEditor !== "undefined", {
    timeout: 20000,
  });
  ...
  await page.waitForSelector(`${containerSelector}.ck-editor__editable`, {
    timeout: 15000,
  });
...
test.describe.skip("CKEditor 5 編輯器功能測試", () => {
  ...
  test.describe("文章編輯器工具列配置", () => {
    test("應該包含完整的工具列項目", async ({ adminPage: page }) => {
      ...
      // 檢查核心編輯工具（粗體、斜體、底線）
      const boldBtn = toolbar
        .locator("button")
        .filter({ hasText: /粗體|Bold|B/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Bold"]'))
        .first();
      await expect(boldBtn).toBeVisible({ timeout: 5000 });

      const italicBtn = toolbar
        .locator("button")
        .filter({ hasText: /斜體|Italic|I/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Italic"]'))
        .first();
      await expect(italicBtn).toBeVisible({ timeout: 5000 });

      const underlineBtn = toolbar
        .locator("button")
        .filter({ hasText: /底線|Underline|U/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Underline"]'))
        .first();
      ...
      // 檢查列表工具
      const bulletListBtn = toolbar
        .locator("button")
        .filter({ hasText: /項目符號|Bullet/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Bullet"]'))
        .first();
      await expect(bulletListBtn).toBeVisible({ timeout: 5000 });

      const numberListBtn = toolbar
        .locator("button")
        .filter({ hasText: /編號|Number/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Number"]'))
        .first();
    ...
    test("應該包含進階格式化工具", async ({ adminPage: page }) => {
      ...
      // 檢查字型大小選單
      const fontSizeDropdown = toolbar
        .locator(".ck-font-size-dropdown")
        .first();
      await expect(fontSizeDropdown).toBeVisible({ timeout: 5000 });

      // 檢查字型家族選單
      const fontFamilyDropdown = toolbar
        .locator(".ck-font-family-dropdown")
        .first();
      await expect(fontFamilyDropdown).toBeVisible({ timeout: 5000 });

      // 檢查對齊選單
      const alignmentDropdown = toolbar
  ...
  test.describe("文章編輯器編輯功能", () => {
    ...
    test("應該能夠插入連結", async ({ adminPage: page }) => {
      ...
      // 點擊連結按鈕
      const linkBtn = toolbar
        .locator("button")
        .filter({ hasText: /連結|Link/ })
        .or(toolbar.locator('[data-cke-tooltip-text*="Link"]'))
        .first();
      await linkBtn.click();
      ...
      const urlInput = page
        .locator('.ck-balloon-panel input[type="text"]')
        .first();
    ...
    test("應該能夠使用鍵盤快捷鍵（Ctrl+B 粗體）", async ({
      adminPage: page,
    }) => {
      await page.goto("/admin/posts/create");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("編輯器內容儲存", () => {
    test("應該能夠建立並儲存文章", async ({ adminPage: page }) => {
      ...
      const saveButton = page.locator(
        'button#submit-btn, button:has-text("發布文章")',
      );
      ...
      const hasSuccessToast = await page
        .locator('.toast, [class*="toast"], text=成功')
        .isVisible()
        .catch(() => false);
