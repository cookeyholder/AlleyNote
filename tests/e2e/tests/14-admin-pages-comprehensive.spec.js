 ...
 */
test.describe.skip("管理後台頁面完整性測試", () => {
  // 所有管理頁面的定義
  const adminPages = [
  ...
  for (const adminPage of adminPages) {
    test(`${adminPage.name} - 頁面應該正常載入`, async ({ page }) => {
      ...
      // 確認頁面標題存在
      const heading = page
        .locator(`h1:has-text("${adminPage.heading}")`)
        .first();
    ...
    test(`${adminPage.name} - 側欄應該可見且有使用者選單`, async ({ page }) => {
      ...
      const userMenu = page.locator(
        '#user-menu-btn, [aria-label*="使用者選單"], button:has-text("admin")',
      );
      await expect(userMenu.first()).toBeVisible({ timeout: 5000 });
    });

    test(`${adminPage.name} - 頁面應該沒有 JavaScript 錯誤`, async ({
      page,
    }) => {
      const errors = [];
...
test.describe("管理頁面核心功能測試", () => {
  ...
  test("文章管理 - 應該有搜尋和篩選功能", async ({ page }) => {
    ...
    const searchInput = page.locator(
      '#search-input, input[placeholder*="搜尋"]',
    );
  ...
  test("文章管理 - 應該有新增文章按鈕", async ({ page }) => {
    ...
    const createBtn = page.locator(
      '#create-post-btn, button:has-text("新增"), button:has-text("建立")',
    );
