...
test.describe("使用者管理功能測試", () => {
  ...
  test("點擊新增使用者應該顯示新增對話框", async ({ adminPage: page }) => {
    ...
    await expect(
      page.locator('button[type="submit"]:has-text("確認建立帳號")'),
    ).toBeVisible();
  });
  ...
  test("新增使用者時密碼與確認密碼應該一致", async ({ adminPage: page }) => {
    ...
    // 應該顯示錯誤訊息
    await expect(page.locator("text=/密碼.*不符/")).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test("點擊編輯應該顯示編輯對話框", async ({ adminPage: page }) => {
    // 等待使用者列表載入
    const editButtons = page.locator('button[title*="編輯"]');
    ...
    if (count > 0) {
      ...
      await expect(
        page.locator('button:has-text("儲存變更內容")'),
      ).toBeVisible();
    }
  });
  ...
  test("每個使用者都應該有編輯和刪除按鈕", async ({ adminPage: page }) => {
    ...
    if (rowCount > 0) {
      ...
      await expect(firstRow.locator('button[title*="移除"]')).toBeVisible();
    }
  });

  test("點擊刪除應該顯示確認對話框", async ({ adminPage: page }) => {
    const deleteButtons = page.locator('button[title*="移除"]');
