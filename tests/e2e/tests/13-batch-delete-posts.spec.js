...
test.describe("批次刪除文章功能", () => {
  ...
  test.skip("應該能使用全選功能", async ({ page }) => {
    // 進入批次模式
    await page.click("#batch-delete-btn");
    await page.waitForTimeout(500);
  ...
  test.skip("應該能退出批次模式", async ({ page }) => {
    // 進入批次模式
    await page.click("#batch-delete-btn");
    await page.waitForTimeout(500);
  ...
  test.skip("未選擇文章時點擊刪除應該顯示錯誤", async ({ page }) => {
    // 進入批次模式
    await page.click("#batch-delete-btn");
    await page.waitForTimeout(500);
  ...
  test.skip("應該能成功批次刪除文章", async ({ page }) => {
    ...
    const confirmBtn = page.locator(
      'button:has-text("確定"), button:has-text("刪除")',
    );
  ...
  test.skip("批次模式下不應該顯示操作按鈕", async ({ page }) => {
    // 進入批次模式
    await page.click("#batch-delete-btn");
    await page.waitForTimeout(500);
    ...
    const editBtnInTable = await firstRow
      .locator('button:has-text("編輯")')
      .count();
    const deleteBtnInTable = await firstRow
  ...
  test.skip("一般模式下應該顯示操作按鈕", async ({ page }) => {
    // 確保不在批次模式
    const toolbar = page.locator("#batch-toolbar");
