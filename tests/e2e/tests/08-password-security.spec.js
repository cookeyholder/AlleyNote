...
test.describe("密碼安全性測試", () => {
  ...
  test("弱密碼應該被拒絕 - 太短", async ({ adminPage: page }) => {
    ...
    // 應該看到錯誤訊息
    await expect(
      page.locator(".requirement-item").filter({ hasText: "至少 8 個字元" }),
    ).toHaveClass(/text-modern-600/);
  });
  ...
  test("弱密碼應該被拒絕 - 缺少大寫字母", async ({ adminPage: page }) => {
    ...
    // 檢查要求清單
    await expect(
      page.locator(".requirement-item").filter({ hasText: "包含大寫字母" }),
    ).toHaveClass(/text-modern-600/);
  ...
  test("弱密碼應該被拒絕 - 包含連續字元", async ({ adminPage: page }) => {
    ...
    // 應該檢測到連續字元 (abc)
    await expect(
      page.locator(".requirement-item").filter({ hasText: "不包含連續字元" }),
    ).toHaveClass(/text-modern-600/);
  });
  ...
  test.skip("弱密碼應該被拒絕 - 包含重複字元", async ({ adminPage: page }) => {
    ...
    // 應該檢測到重複字元 (aaa)
    await expect(
      page.locator(".requirement-item").filter({ hasText: "不包含重複字元" }),
    ).toHaveClass(/text-modern-600/);
  });
  ...
  test.skip("強密碼應該被接受", async ({ adminPage: page }) => {
    ...
    await expect(
      page.locator(".requirement-item").filter({ hasText: "包含小寫字母" }),
    ).toHaveClass(/text-green-600/);
    await expect(
      page.locator(".requirement-item").filter({ hasText: "包含大寫字母" }),
    ).toHaveClass(/text-green-600/);
    await expect(
      page.locator(".requirement-item").filter({ hasText: "包含數字" }),
    ).toHaveClass(/text-green-600/);
    await expect(
      page.locator(".requirement-item").filter({ hasText: "包含特殊符號" }),
    ).toHaveClass(/text-green-600/);
  ...
  test.skip("密碼包含使用者名稱應該被警告", async ({ adminPage: page }) => {
    ...
    // 應該警告密碼包含使用者名稱
    await expect(page.locator(".password-strength-indicator")).toContainText(
      "使用者名稱",
    );
  });
