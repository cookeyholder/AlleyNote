...
test.describe('角色管理頁面', () => {
  ...
  test('應該成功新增角色', async ({ page }) => {
    ...
    // 清理：刪除測試角色
    await page.locator(`.role-item:has-text("${displayName}") button:has-text("刪除")`).click();
  ...
  test('應該能選擇角色並顯示權限設定', async ({ page }) => {
    ...
    // 檢查是否有取消按鈕
    await expect(permissionsContainer.locator('button:has-text("取消")')).toBeVisible();
  });
  ...
  test('應該能刪除角色', async ({ page }) => {
    ...
    // 檢查角色是否從列表中移除
    await expect(page.locator(`h3:has-text("${displayName}")`)).not.toBeVisible();
  });
  ...
  test('不應該能刪除超級管理員角色', async ({ page }) => {
    // 檢查超級管理員角色項目
    const superAdminRole = page.locator('.role-item:has-text("超級管理員")');
    
    // 超級管理員角色不應該有刪除按鈕
    await expect(superAdminRole.locator('button:has-text("刪除")')).not.toBeVisible();
  });
});
