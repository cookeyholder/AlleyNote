...
async function hasSettingsBackendIssue(page) {
  const issueLocator = page.locator(
    "text=/no such table: settings|載入設定失敗|SQLSTATE\\[HY000\\]/i",
  );
  return (await issueLocator.count()) > 0;
}

/**
 * 系統設定頁面功能測試
 */
test.describe.skip("系統設定頁面測試", () => {
  ...
  test("應該能夠成功儲存設定", async ({ page }) => {
    ...
    // 檢查成功訊息
    const successToast = page.locator("text=/設定已儲存/i, text=/成功/i");
  ...
  test("儲存後重新載入應該保留設定", async ({ page }) => {
    ...
    test.skip(
      await hasSettingsBackendIssue(page),
      "目前環境設定資料表不可用，略過持久化驗證",
    );
  ...
  test("切換多個開關應該正常運作", async ({ page }) => {
    ...
    const registrationLabel = page.locator("label:has(#enable-registration)");
