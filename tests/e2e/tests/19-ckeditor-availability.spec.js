...
test.describe.skip("CKEditor 編輯器可用性測試", () => {
  test("新增文章頁面的 CKEditor 應該正常工作", async ({ adminPage: page }) => {
    ...
    const boldButton = toolbar
      .locator("button")
      .filter({ hasText: /Bold/i })
      .first();
    await expect(boldButton).toBeVisible();

    const italicButton = toolbar
      .locator("button")
      .filter({ hasText: /Italic/i })
      .first();
  ...
  test("編輯文章頁面的 CKEditor 應該正常工作", async ({ adminPage: page }) => {
    ...
    const editButton = page
      .locator(`tr:has-text("${testTitle}") button:has-text("編輯")`)
      .first();
    ...
    const deleteButton = page
      .locator(`tr:has-text("${testTitle}") button:has-text("刪除")`)
      .first();
  ...
  test("CKEditor 不應該顯示 toolbarview-item-unavailable 警告", async ({
    adminPage: page,
  }) => {
    // 設置 console 監聽
    const warnings = [];
    page.on("console", (msg) => {
      if (
        msg.type() === "warning" &&
        msg.text().includes("toolbarview-item-unavailable")
      ) {
