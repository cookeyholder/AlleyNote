...
class JwtAuthenticationIntegrationTest extends TestCase
{
    ...
    public function testCanLogoutAndBlacklistTokens(): void
    {
        ...
        $this->assertGreaterThan(0, (is_array($stats) && isset((is_array($stats) ? $stats['total_blacklisted'] : (is_object($stats) ? $stats->total_blacklisted : null)))) ? (is_array($stats) ? $stats['total_blacklisted'] : (is_object($stats) ? $stats->total_blacklisted : null)) : null);
    }
    ...
    public function testCanCleanupExpiredBlacklistEntries(): void
    {
        ...
        $cleanedCount = (is_array($cleanupResult) ? $cleanupResult['expired_cleaned'] : (is_object($cleanupResult) ? $cleanupResult->expired_cleaned : null)) ?? 0;
    ...
    public function testCanPerformHealthCheck(): void
    {
        ...
        $this->assertGreaterThanOrEqual(1, (is_array($healthStatus) && isset((is_array($healthStatus) ? $healthStatus['total_entries'] : (is_object($healthStatus) ? $healthStatus->total_entries : null)))) ? (is_array($healthStatus) ? $healthStatus['total_entries'] : (is_object($healthStatus) ? $healthStatus->total_entries : null)) : null);
    }
