...
AlleyNote 採用 **安全導向的領域驅動設計 (Security-Oriented DDD, 簡稱 Secure-DDD)**，將安全性視為系統的第一類公民：

1. **安全左移 (Security Left)**：資安邏輯（如 IP 辨識、HTML 淨化、密碼強度驗證）從控制器移至 `Shared` 或 `Infrastructure` 層，確保所有領域操作預設即安全。
2. **標準化淨化介面**：透過 `OutputSanitizerInterface` 結合 `HTMLPurifier` 與 `DOMPurify` 實現多層次防禦，解決富文本過度轉義或注入問題。
3. **嚴謹的測試基類**：所有測試均繼承自 `SecureDDDTestCase`，提供自動化安全性 Mocking 與回應格式斷言。
...
> 備註：開發環境預設使用 `API_HOST_PORT=8081`（避免多個 devcontainer 專案衝突）。
> 若是單一正式環境或不會有衝突，可改成 `API_HOST_PORT=8080`。
...
- **API 文件**：http://localhost:${API_HOST_PORT:-8081}/api/docs
- **健康檢查**：http://localhost:${API_HOST_PORT:-8081}/api/health

#### 埠對照（避免混淆）

- **DevContainer 本機服務模式**（`postStartCommand` 啟動）
  - 前端：`http://localhost:3000`
  - API：`http://localhost:8081`
- **Docker Compose 模式**（本專案預設）
  - 前端：`http://localhost:3000`
  - API：`http://localhost:${API_HOST_PORT:-8081}`（預設 `8081`；正式可設 `API_HOST_PORT=8080`）
...
| 主管理員 | superadmin@example.com | SuperAdmin@123456 |
...
為避免文件與程式碼示例出現不一致，請統一遵循以下規則：

1. **預設管理員帳號（僅限真實啟動指引）**
   - 帳號：admin@example.com
   - 密碼：Admin@123456
   - 帳號：superadmin@example.com
   - 密碼：SuperAdmin@123456
   - 僅用於「快速開始 / 登入驗證 / 重設管理員」等實際操作教學。

2. **一般 API / 程式碼示例（非預設帳號語境）**
   - 一律使用：Example#Pass123!
   - 禁止再使用：password、password123、securepassword123、TestPassword@123、MySecure@Pass123 等舊示例。

3. **弱密碼教學例外**
   - 在「密碼安全」與「黑名單」說明中，可保留 Password123、password 等作為反例。
