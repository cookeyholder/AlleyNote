...
- **SQLite in-memory 測試一致性**：明確規範 `ApiTestCase`、DSL 資料庫斷言與應用容器必須共用同一個 PDO 連線，避免「測試查到的 DB 與 API 寫入的 DB 不同」。
- **DSL 驗證矩陣**：新增 `assertDatabaseHas()` / `assertDatabaseMissing()` 在 create/delete/rollback/巢狀交易場景下的最小可行測試矩陣。
...
- **Testing Runtime**: `ApiTestCase` 與 DI 容器的 PDO 綁定策略會調整為同一連線來源，降低 in-memory 測試誤判風險。
