...
// 追蹤 MutationObserver 以便在銷毀編輯器時一併清理
...
export async function initRichTextEditor(elementId, options = {}) {
    ...
    if (typeof CKEDITOR === "undefined") {
        throw new Error(
            "CKEditor 5 尚未載入，請先在 HTML 中引入 CKEditor 5 CDN"
        );
    }
    ...
    try {
        ...
        const applyEditableId = () => {
            const editableElement = editor?.ui?.getEditableElement?.();
            if (editableElement && editableElement.id !== elementId) {
                editableElement.setAttribute("id", elementId);
            }
        ...
        const editableElement = applyEditableId();

        const observerTarget =
            editableElement?.parentElement || editor?.ui?.view?.element;
        if (observerTarget) {
            ...
            editorObservers.set(elementId, observer);
        }
...
export async function destroyRichTextEditor(elementId) {
    ...
    const observer = editorObservers.get(elementId);
    if (observer) {
        observer.disconnect();
        editorObservers.delete(elementId);
    }
}
