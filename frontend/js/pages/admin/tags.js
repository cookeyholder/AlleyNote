...
export default class TagsPage {
  ...
  renderTagCard(tag) {
    ...
    return `
      <div class="bg-white rounded-xl border-2 ${color.border} p-6 hover:shadow-lg transition-shadow">
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1">
            <h3 class="text-lg font-semibold ${color.text} mb-2">${this.escapeHtml(tag.name)}</h3>
            ${tag.description ? `
              <p class="text-sm text-modern-600">${this.escapeHtml(tag.description)}</p>
            ` : ''}
          </div>
