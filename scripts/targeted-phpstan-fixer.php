...
class SpecificPhpstanFixer
{
    ...
    private function getErrorFiles(): array
    {
        // 從當前目錄結構推斷需要修復的檔案
        return [
    ...
    private function processFile(string $filePath): void
    {
        ...
        // 1. 修復方法參數中缺少 array value type 的問題
        ...
        // 4. 修復 array return type 缺少 value type
    ...
    private function fixArrayParameterTypes(string $content, bool &$hasChanges): string
    {
        ...
        $patterns = [
            // 修復 $args 參數 (常用於路由參數)
            '/function\s+(\w+)\([^)]*array\s+\$args[^)]*\)/' => function ($matches) use (&$hasChanges) {
                $replacement = str_replace('array $args', 'array<string, mixed> $args', $matches[0]);
            ...
            // 修復其他常見的 array 參數
            '/function\s+(\w+)\([^)]*array\s+\$(\w+)[^)]*\)/' => function ($matches) use (&$hasChanges) {
                $paramName = $matches[2];
                $typeHints = [
                ...
                $type = $typeHints[$paramName] ?? 'array<string, mixed>';
                $replacement = str_replace("array \${$paramName}", "{$type} \${$paramName}", $matches[0]);
    ...
    {
        // 修復方法回傳類型中的 array 缺少 value type
        $patterns = [
            // 修復 ): array 改為具體的 array type
            '/function\s+(\w+)\([^)]*\):\s*array\s*\{/' => function ($matches) use (&$hasChanges) {
                ...
                $returnTypes = [
                    ...
                    'getList' => 'array<string, mixed>',
                ];
                
                $type = $returnTypes[$methodName] ?? 'array<string, mixed>';
                $replacement = str_replace(': array {', ": {$type} {", $matches[0]);
    ...
    private function fixPropertyTypes(string $content, bool &$hasChanges): string
    {
        ...
        $patterns = [
            // 修復 private array 屬性
            '/private\s+array\s+\$(\w+);/' => function ($matches) use (&$hasChanges) {
                $propName = $matches[1];
                $typeHints = [
                    'headers' => 'array<string, string>',
                    'attributes' => 'array<string, mixed>',
                ...
                $type = $typeHints[$propName] ?? 'array<string, mixed>';
                $hasChanges = true;
                $this->fixCount++;
                return "private {$type} \${$propName};";
            },
            
            // 修復 protected array 屬性
            '/protected\s+array\s+\$(\w+);/' => function ($matches) use (&$hasChanges) {
                $propName = $matches[1];
                $type = 'array<string, mixed>'; // 預設類型
                $hasChanges = true;
                $this->fixCount++;
