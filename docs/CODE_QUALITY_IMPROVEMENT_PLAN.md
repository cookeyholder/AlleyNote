...
**具體任務**:
- [ ] 為 70+ 個腳本檔案添加適當的命名空間 `AlleyNote\Scripts\`
- [ ] 重構 `scan-project-architecture.php` 等核心腳本
- [ ] 更新 composer.json 自動載入配置
- [ ] 建立 scripts/lib/ 子目錄結構
...
- [ ] 掃描所有 PHP 檔案檢查類別名稱
- [ ] 修復不一致的檔案 (預估 15-20 個)
- [ ] 確保所有檔案有 `declare(strict_types=1)` 聲明
...
**具體任務**:
- [ ] 新增 `HttpStatusCode` 枚舉
- [ ] 新增 `CacheType` 枚舉
- [ ] 新增 `LogLevel` 枚舉
- [ ] 新增 `DatabaseAction` 枚舉
- [ ] 新增 `SecurityLevel` 枚舉
