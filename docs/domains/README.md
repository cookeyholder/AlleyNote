...
> 本目錄記錄 AlleyNote 的 7 個 Bounded Context（BC），
> 說明各自職責、核心檔案、依賴方向與適合讀者。

| # | BC | 職責 | 適合讀者 |
|---|----|------|----------|
| [01](01-認證授權領域.md) | **Auth** — 認證與授權 | JWT 認證、RBAC + 直接權限授權、密碼管理、角色/權限 CRUD | 後端開發者、安全工程師 |
| [02](02-文章領域.md) | **Post** — 公告 | 公告 CRUD、標籤管理、內容審查、快取失效、搜尋、Markdown 匯出 | 後端開發者、內容管理者 |
...
| [06](06-設定領域.md) | **Setting** — 設定 | 系統設定 key-value 存取與全域參數調控 | 後端開發者 |
| [07](07-通知領域.md) | **Notification** — 站內通知 | 使用者通知列表、未讀計數、已讀狀態更新、全區廣播通知 | 後端開發者、前端開發者 |
...
Statistics ──→ Shared (快取/事件)
  ↑
Notification ──→ Auth (使用者權限)
Notification ──→ Shared (資料庫抽象)
  ↑
