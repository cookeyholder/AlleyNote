...
**API 版本**: RESTful API v5.0
**更新日期**: 2025-09-07
**前後端分離**: Vite + JavaScript + PHP 8.4.11 DDD 後端
...
9. [統計 API](#統計-api)
10. [使用者活動記錄 API](#使用者活動記錄-api)
11. [API 文件產生](#api-文件產生)
12. [錯誤代碼](#錯誤代碼)
13. [速率限制](#速率限制)
...
AlleyNote API v5.0 提供完整的前後端分離筆記管理系統功能，基於 PHP 8.4.11 DDD 架構設計，包含筆記管理、統計分析、使用者認證、附件上傳、安全控制等功能。

### 版本 5.0 特色

- ✅ **前後端分離**: Vite + JavaScript + PHP 8.4.11 DDD 後端
- ✅ **DDD 架構**: 5個領域模組，429個類別，80個介面
- ✅ **完整統計功能**: 使用者活動、筆記統計、效能監控
- ✅ **現代化容器**: Docker 24.0+ & Docker Compose v2.20+
- ✅ **強型別驗證**: PHP 8.4 型別系統，統一 DTO 驗證
- ✅ **設計模式**: 29個Repository、84個Service、172個依賴注入
- ✅ **自動 API 文件**: Swagger 整合，自動產生 OpenAPI 規格
- ✅ **增強安全性**: CSRF 防護、XSS 過濾、活動監控
- ✅ **效能優化**: 多層快取、查詢優化、可維護性 100/100
- ✅ **快取系統**: 分層快取管理、標籤群組化

### 技術堆疊

- **後端**: PHP 8.4.11 (現代化語法特性)
...
- **總類別數**: 429 個
- **總介面數**: 80 個
- **設計模式**: Repository(29)、Service(84)、Factory(8)、Command(3)、DI(172)
