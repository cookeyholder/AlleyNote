...
class PasswordSecurityService implements PasswordSecurityServiceInterface
{
    ...
    public function validatePassword(string $password): void
    {
        ...
        if ($commonPasswordResult['is_common']) {
            throw new ValidationException($commonPasswordResult['message']);
        }

        // 檢查重複字元
        if ($this->hasExcessiveRepetition($password)) {
            throw new ValidationException('密碼包含過多重複字元');
        }

        // 檢查順序字元
        if ($this->hasSequentialChars($password)) {
            throw new ValidationException('密碼不能包含連續的字元序列');
        }
    }
    ...
    public function calculatePasswordStrength(string $password): array
    {
        ...
        if (!$hasUpper) $feedback[] = '建議包含大寫字母';
        if (!$hasLower) $feedback[] = '建議包含小寫字母';
        if (!$hasNumber) $feedback[] = '建議包含數字';
        if (!$hasSymbol) $feedback[] = '建議包含特殊符號';
    ...
    private function validatePasswordComplexity(string $password): void
    {
        ...
        if (!empty($errors)) {
            throw new ValidationException('密碼必須' . implode('、', $errors));
        }
    }
        ...
        if ($pwnedResult['api_available']) {
            if ($pwnedResult['is_leaked']) {
                return [
                    'is_common' => true,
                    'message' => sprintf(
                        '此密碼已在 %d 次資料外洩事件中被發現，請選擇一個更安全的密碼',
                        $pwnedResult['count']
                    ),
                    'source' => 'hibp_api',
