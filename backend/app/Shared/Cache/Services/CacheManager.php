...
class CacheManager implements CacheManagerInterface
{
    ...
    /** @var TagRepositoryInterface|null 標籤倉庫 */
    ...
    public function remember(string $key, callable $callback, int $ttl = 3600): mixed
    {
        ...
        try {
            ...
            $this->logger->error('Remember 快取失敗', [
                'key' => $key,
                'error' => $e->getMessage(),
            ]);
    ...
    public function tags(string|array $tags): TaggedCacheInterface
    {
        ...
        // 如果沒有找到支援標籤的驅動，使用 TaggedCacheManager
    ...
    {
        if (!($this->config['enable_sync'] ?? false)) {
            return;
        }
    ...
    private function handleDriverError(string $driverName, Exception $error, string $operation, array $params): mixed
    {
        ...
        $availableDrivers = array_filter(
            $this->drivers,
            fn($driver, $name): bool => $driverName && $driver->isAvailable(),
