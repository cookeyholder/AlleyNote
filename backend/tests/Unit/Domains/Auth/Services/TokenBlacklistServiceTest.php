...

{
    ...
    public function testGetHealthStatusUnhealthyWithRecommendations(): void
    {
        ...
        $this->assertSame(100000, $result['max_recommended_size']);
        $this->assertStringContainsString('Run cleanup to reduce blacklist size', implode(' ', (array) $result['recommendations']));
        $this->assertStringContainsString('Blacklist size exceeds recommended limit, consider cleanup', implode(' ', (array) $result['recommendations']));
        $this->assertStringContainsString('High number of expired entries, consider cleanup', implode(' ', (array) $result['recommendations']));
        $this->assertStringContainsString('High security-related blacklist entries detected', implode(' ', (array) $result['recommendations']));
    }
