...
test.describe("系統設定整合測試", () => {
    ...
    test("時區設定應該顯示在文章頁面", async ({ page }) => {
        ...
        // 獲取第一篇已發布的文章的連結
        const firstPostLink = await adminPage
    ...
    test("時區變更後文章顯示時間應該改變", async ({ page }) => {
        ...
        if ((await firstPostLink.count()) > 0) {
            ...
            await page.goto(`http://localhost:3000${postUrl}`);
            await page.waitForLoadState("networkidle");
            const utcTime = await page
                .locator("article header time")
                .textContent();
            const utcTimezone = await page
            ...
            await page.goto(`http://localhost:3000${postUrl}`);
            await page.waitForLoadState("networkidle");
            const tokyoTime = await page
                .locator("article header time")
                .textContent();
            const tokyoTimezone = await page
                .locator("article header")
                .textContent();
