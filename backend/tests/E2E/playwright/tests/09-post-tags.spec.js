...
test.describe("文章標籤管理", () => {
    ...
    test("應該能在編輯文章時添加和移除標籤", async ({ page }) => {
        ...
        const createButton = page
            .locator('#create-post-btn, a[href="/admin/posts/create"]')
            .first();
        ...
        if (options > 1) {
            ...
            const selectedTags = page.locator(
                "#tags-container span.inline-flex"
            );
    ...
    test("應該能創建帶有多個標籤的文章", async ({ page }) => {
        ...
        if (tagCount < 2) {
            for (let i = tagCount; i < 2; i++) {
                ...
                const uniqueTagName = `測試標籤_${Date.now()}_${i}_${Math.floor(
                    Math.random() * 1000
                )}`;
