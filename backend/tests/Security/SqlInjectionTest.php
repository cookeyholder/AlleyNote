...
class SqlInjectionTest extends TestCase
{
    ...
    public function shouldSanitizeSearchInput(): void
    {
        ...
        foreach ($maliciousInputs as $maliciousInput) {
            ...
            $this->assertLessThanOrEqual(3, $results['total'] ?? 0, "SQL injection should not return all data: {$maliciousInput}");
        }
    ...
    public function shouldPreventUnionBasedAttacks(): void
    {
        ...
        foreach ($unionAttacks as $attack) {
            ...
            // UNION 攻擊不應該成功
            $this->assertLessThanOrEqual(3, (is_array($results) && array_key_exists('total', $results) ? $results['total'] : null), "Union attack should be prevented: {$attack}");
        }
    }
    ...
    public function shouldPreventBooleanBasedBlindAttacks(): void
    {
        ...
        foreach ($booleanAttacks as $attack) {
            ...
            $this->assertLessThanOrEqual(3, (is_array($results) && array_key_exists('total', $results) ? $results['total'] : null), "Boolean attack should be prevented: {$attack}");
        }
    }
