...
### 步驟 1：建立分支和準備工作
- [ ] 1.1 建立新分支 `feature/frontend-backend-separation`
- [ ] 1.2 確保當前程式碼狀態乾淨（沒有未提交的變更）
- [ ] 1.3 建立備份點（創建 tag）
...
- [ ] 2.1 建立 `backend/` 目錄
- [ ] 2.2 建立 `frontend/` 目錄
- [ ] 2.3 建立 `frontend/src/` 目錄
- [ ] 2.4 建立 `frontend/public/` 目錄
- [ ] 2.5 建立臨時備份目錄 `_migration_backup/`
...
- [ ] 3.1 搬移 `app/` → `backend/app/`
- [ ] 3.2 搬移 `config/` → `backend/config/`
- [ ] 3.3 搬移 `database/` → `backend/database/`
- [ ] 3.4 搬移 `tests/` → `backend/tests/`
- [x] 3.5 搬移 `storage/` → `backend/storage/` ✅ **已完成**
- [ ] 3.6 搬移 `composer.json` → `backend/composer.json`
- [ ] 3.7 搬移 `composer.lock` → `backend/composer.lock`
- [ ] 3.8 搬移 `phpunit.xml` → `backend/phpunit.xml`
- [ ] 3.9 搬移 `phinx.php` → `backend/phinx.php`
- [ ] 3.10 搬移所有 `phpstan*.neon` 檔案 → `backend/`
- [ ] 3.11 搬移 `.php-cs-fixer.*` 檔案 → `backend/`
...
- [ ] 4.1 搬移 `public/` → `frontend/public/`
- [ ] 4.2 建立 `frontend/src/` 基本結構
- [ ] 4.3 建立 `frontend/package.json`
- [ ] 4.4 建立 `frontend/vite.config.js`
...
### 步驟 5：更新 Docker 配置
- [ ] 5.1 更新 `docker-compose.yml` 路徑參考
- [ ] 5.2 更新 `docker-compose.production.yml` 路徑參考
- [ ] 5.3 更新 `docker-compose.test.yml` 路徑參考
- [ ] 5.4 更新 `docker/nginx/` 配置檔案
- [ ] 5.5 更新 `docker/php/` Dockerfile
